...
class Onshape():
    ...
    def __init__(self, credentials, logging=False):
        ...
        if self._logging:
            utils.log('onshape instance created: url = %s, access key = %s' % (self._creds.url, self._creds.access_key))
    ...
    def _make_auth(self, method, date, nonce, path, query={}, ctype='application/json'):
        ...
        signature = base64.b64encode(hmac.new(self._creds.secret_key.encode('utf-8'), hmac_str, digestmod=hashlib.sha256).digest())
        auth = 'On ' + self._creds.access_key + ':HmacSHA256:' + signature.decode('utf-8')
    ...
    def request(self, method, path, query={}, headers={}, body={}, base_url=None):
        ...
        if res.status_code == 307:
            ...
            raise Exception("request failed, details: " + res.text)
        else:
            if self._logging:
